...
class inventory(commands.Cog):
    ...
    async def inventory(self, ctx, user:discord.Member=None):
        ...
        cursor.execute(f'SELECT code, game, team, player, teamyear, rarity, issue FROM UserCards WHERE userid={userid} ORDER BY rarity DESC, team ASC, player ASC, issue ASC LIMIT 20;')
        cardlist = cursor.fetchall()
        desc_title = f'{username} has **{len(allcardlist)}** cards in their inventory!'
        if len(cardlist) == 0:
            ...
            for i in cardlist:
                desc = desc + f'\n` {i[5]}⭐ `—` {i[2]} `—` {i[3]} `—` {i[0]}#{i[6]} `'
            embed = discord.Embed(description=f'{desc_title}\n{desc}', colour=discord.Color.from_rgb(0,0,0))
